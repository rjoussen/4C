...
namespace TimeStepping
{
  ...
  TimeStepControlSettings::TimeStepControlSettings(
      ...
      : decrease_factor(input.decrease_factor),
        min_timestep(initial_time_step * input.min_timestep_ratio),
        steps_to_increase(input.steps_to_increase),
        max_timestep(input.max_timestep.value_or(initial_time_step)),
  ...
  [[nodiscard]] Core::IO::InputSpec TimeStepControlSettings::input_spec()
  {
    ...
    return group<InputParameters>("TIME STEP CONTROL",
        {
            ...
            parameter<double>("MIN_TIME_STEP_RATIO",
                {.description = "Minimum allowed global time-step size during "
                    ...
                    .validator = Validators::in_range(Validators::excl(0.0), Validators::excl(1.0)),
                    .store =
                        in_struct(&TimeStepControlSettings::InputParameters::min_timestep_ratio)}),
            parameter<int>("STEPS_TO_INCREASE",
                {.description = "Number of consecutively accepted time steps on a "
                                "reduced time-step size before "
                                "increasing the global time-step size again",
                    .default_value = 4,
            ...
            parameter<std::optional<double>>("MAX_AVERAGE_NONLINEAR_ITERATIONS",
                {.description =
                        ...
                        "iterations "
                        "over the last STEPS_TO_INCREASE steps is below this value. If "
                        "omitted, time-step increase is attempted regardless of the "
            ...
            parameter<bool>("REDUCE_TO_MAX_TIME",
                {.description = "If true, the last time-step size is reduced to exactly reach the "
                                "specified MAXTIME.",
                    .default_value = false,
                    .store =
  ...
  double compute_reduced_time_step(const double current_dt, const TimeStepControlSettings& settings)
  {
    ...
    if (new_dt < settings.min_timestep)
    {
      FOUR_C_THROW(
          ...
          current_dt, new_dt, settings.min_timestep);
    }
      ...
      const int remaining_steps)
  {
    ...
    // current_dt might be larger than max_timestep if the initial timestep was set larger than
    // max_timestep.
    if (current_dt < settings.max_timestep and
        num_successful_steps_at_current_dt >= settings.steps_to_increase)
    {
      ...
      {
        new_dt = std::min(current_dt * settings.increase_factor, settings.max_timestep);
      }
    }
