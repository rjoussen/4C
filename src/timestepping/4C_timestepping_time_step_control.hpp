...
namespace TimeStepping
{
  struct TimeStepControlSettings
  {
    ...
    struct InputParameters
    {
      ...
      double min_timestep_ratio;
      int steps_to_increase;
      std::optional<double> max_timestep;
      std::optional<double> increase_factor;
      std::optional<double> max_average_nonlinear_iterations;
      bool reduce_to_max_time = false;
    };

    /// Return the input specification for the TIME STEP CONTROL parameter group.
   ...
   * size was selected
   * @param newton_iterations Newton iteration counts from the recent accepted steps
