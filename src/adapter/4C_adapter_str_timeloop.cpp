...
void Adapter::StructureTimeLoop::integrate()
{
  ...
  while (not_finished())
  {
    ...
    const Solid::StepAction solve_action = perform_error_action(solve_status);
    if (solve_action == Solid::StepAction::retry_step) continue;
    if (solve_action == Solid::StepAction::stop_time_loop) break;
