...
namespace
{
  ...
  double evaluate_jacobian_determinant(Core::IO::MeshInput::Mesh& mesh,
      const std::vector<int>& connectivities, const Core::LinAlg::Tensor<double, dim>& xi)
  {
  ...
  TEST(VTU, MeshCubeHex8)
  {
    ...
    EXPECT_EQ(mesh.side_sets.size(), 0);
    EXPECT_EQ(mesh.points.size(), 16);
    EXPECT_EQ(mesh.cell_blocks[1].cell_connectivities.size(), 2);
    EXPECT_EQ(mesh.cell_blocks[2].cell_connectivities.size(), 1);

    // check node ordering of the connectivity by evaluating the jacobian at xi=[0.1,0.2,0.3]
    for (const auto& [_, connectivities] : mesh.cell_blocks[1].cell_connectivities)
    {
      double detJ =
  ...
  TEST(VTU, MeshCubeHex27)
  {
    ...
    // check node ordering of the connectivity by evaluating the jacobian at xi=[0.1,0.2,0.3]
    for (const auto& [id, connectivities] : mesh.cell_blocks[1].cell_connectivities)
    {
      double detJ =
          evaluate_jacobian_determinant<celltype, 3>(mesh, connectivities, {{0.1, 0.2, 0.3}});
  ...
  TEST(VTU, MeshCubeTet4)
  {
    ...
    EXPECT_EQ(mesh.cell_blocks[1].cell_connectivities.size(), 28);
  ...
  TEST(VTU, MeshCubePyramid5)
  {
    ...
    EXPECT_EQ(mesh.cell_blocks[1].cell_connectivities.size(), 6);
