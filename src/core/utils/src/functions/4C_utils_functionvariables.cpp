...
Core::Utils::ParsedFunctionVariable::ParsedFunctionVariable(
    ...
    : FunctionVariable(std::move(name)),
      timefunction_(std::make_shared<Core::Utils::SymbolicExpression<double, "t">>(buf))
{
}
...
double Core::Utils::ParsedFunctionVariable::value(const double t)
{
  ...
  double value = timefunction_->value(var<"t">(t));
    ...
    const double t, const unsigned deg)
{
  ...
  vfad = timefunction_->second_derivative(var<"t">(tfad));
      ...
      t2_(periodicdata.t2)
{
  ...
  {
    timefunction_[n] =
        std::make_shared<Core::Utils::SymbolicExpression<double, "t">>(description_vec[n]);
  }
}
...
double Core::Utils::MultiFunctionVariable::value(const double t)
{
  ...
  if (index == 0)
  {
    value = timefunction_[0]->value(var<"t">(t_equivalent));
  }
  else
  {
    value = timefunction_[index - 1]->value(var<"t">(t_equivalent));
  }
...
double Core::Utils::MultiFunctionVariable::time_derivative_value(const double t, const unsigned deg)
{
  ...
  if (index == 0)
  {
    vfad = timefunction_[0]->second_derivative(var<"t">(tfad));
  }
  else
  {
    vfad = timefunction_[index - 1]->second_derivative(var<"t">(tfad));
  }
