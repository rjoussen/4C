...
namespace Solid
{
  ...
  namespace TimeInt
  {
    ...
    namespace Internal
    {
       ...
       * Core::Mat::TimeStepReduction::request_time_step_reduction(reason) to set a local,
       * non-exceptional request. This exception is thrown only after structural evaluation has
       * consumed that request, MPI-reduced it, and needs to escape NOX without losing the distinct
       * timestep-reduction status.
       */
